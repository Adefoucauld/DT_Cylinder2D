...
class DecisionTransformer(nn.Module):
    ...
    def evaluate_model(
        ...
        state_mean = th.as_tensor(state_mean).to(device=None)
        ...
        target_return = th.tensor(ep_return, device=None, dtype=th.float32).reshape(1, 1)
        timesteps = th.tensor(0, device=None, dtype=th.long).reshape(1, 1)
        ...
        for e in range(E):
            ...
            rewards = th.cat([rewards, th.zeros(1, device=None)])
            ...
            timesteps = th.cat([timesteps, th.ones((1, 1), device=None, dtype=th.long) * (e+1)], dim=1)
